...
interface IOtherQuestionsControllerResponse {
  getters: {
    ...
    thirteenthQuestionValue: string;
  ...
  handlers: {
    ...
    handleThirteenthQuestionChange: (
      event: ChangeEvent<HTMLInputElement>,
    ) => void;
  };
  refs: {
...
export const useOtherQuestionsController =
  (): IOtherQuestionsControllerResponse => {
    ...
    const thirteenthQuestionValue = useAppSelector(THIRTEENTH_QUESTION);
    ...
    /**
     * Handle change of Thirteenth Question's Radios option
     * @function handleThirteenthQuestionChange
     * @param {ChangeEvent<HTMLInputElement>} event
     */
    const handleThirteenthQuestionChange = (
      event: ChangeEvent<HTMLInputElement>,
    ): void => {
      dispatch(
        form1Actions.setThirteenthQuestion(
          (event.target as HTMLInputElement).value,
        ),
      );
    };
